...
# /dep-manager:mise-ci — CI に mise-action を組み込む
...
- **作業範囲は対象ジョブへの mise-action 組み込みとその周辺**。ワークフロー全体の再構成(ジョブ分割、トリガー変更、キャッシュ戦略の刷新等)はしない。
- 手順5で見つけた**未 pin のアクションは指摘までに留める**。修正はユーザーの確認を得てから行う。
