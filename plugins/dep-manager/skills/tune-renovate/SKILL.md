...
# /dep-manager:tune-renovate — Renovate 設定のチューニング
...
1. **現状の読み取り**: `renovate.json`(無ければ `.renovaterc(.json)`)を Read。存在しなければ `/dep-manager:migrate-dependabot` を案内して終了。
...
```bash
/dep-manager:tune-renovate cooldown 3日 と minor/patch の自動マージを追加
/dep-manager:tune-renovate eslint 関連を 1 PR にグルーピング
```
