...
# /renovate:enable-mise-manager — mise を Renovate の更新対象にする
...
- リポジトリに `.mise.toml`(または `mise.toml` / `.tool-versions`)が存在すること。無ければ `mise` プラグインの `/mise:mise-sync` を先に案内する。
...
- `mise` で管理するツールに `minimumReleaseAge` を効かせたい場合は `/renovate:tune-renovate` で cooldown を設定する。
...
- `.mise.toml` 自体の整備・CI 連携は `mise` プラグイン(`/mise:mise-sync` / `/mise:mise-ci`)を参照。
