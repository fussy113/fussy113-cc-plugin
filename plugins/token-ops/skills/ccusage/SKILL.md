...
# /token-ops:ccusage — トークン消費・コストの可視化

[ccusage](https://github.com/ryoppippi/token-ops:ccusage) を呼び出し、Claude Code のトークン消費とコストを集計表示します。
...
- `npx` が見つからない → 「Node.js/npm 環境が必要です。`dep-manager` の mise スキルや Node のインストールを確認してください」と案内。
